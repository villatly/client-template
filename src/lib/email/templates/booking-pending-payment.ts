...
export function renderBookingPendingPayment(
  ...
  const previewText = `Your dates are held — complete payment within 23 hours to confirm your stay at ${config.name}.`;
  ...
  const body = `
    ...
    <p style="margin:0 0 28px;font-size:13px;color:#a8a29e;">
      ⏱ This payment link is valid for <strong>23 hours</strong>.
    </p>
  ...
  const text = [
    ...
    ``,
    `Your dates at ${config.name} are being held. Complete payment within 23 hours to confirm your reservation.`,
    ``,
    `BOOKING SUMMARY`,
